...
public class Zombie extends Sprite{
    ...
    void follow(Vector2D target){
        ...
        for(Building building : buildings){
            if(Barrier.intersectsLine(location.x, location.y, target.x, target.y, building.getLayoutX(), building.getLayoutY(), building.getWidth(), building.getHeight())){
                points.addAll(building.getBypassPoints(location, target));
        ...
        points.sort(Comparator.comparingDouble(c -> Vector2D.subtract(location, c).magnitude()));
